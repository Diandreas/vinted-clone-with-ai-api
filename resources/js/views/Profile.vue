<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white">
    ...
    <div class="pt-16 sm:pt-20 md:pt-24 lg:pt-28">
      ...
      <!-- Stories Section -->
      <div class="max-w-4xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6">
        <StoriesGrid :stories="stories" :user-id="user?.id" />
      </div>
        ...
        <div v-else class="space-y-4 sm:space-y-6">
          ...
          <div v-if="activeTab === 'lives'" class="space-y-4 sm:space-y-6">
            <div v-if="lives.length === 0" class="text-center py-12 sm:py-16">
              ...
              <p class="text-gray-500 text-sm sm:text-base">Vous n'avez pas encore créé de lives.</p>
              <RouterLink
                to="/lives/create"
                class="inline-block mt-4 px-6 py-2 bg-red-600 hover:bg-red-700 rounded-full transition-colors text-sm sm:text-base"
              >
...
async function fetchStories() {
  try {
    const resp = await window.axios.get(`/users/${authStore.user.id}/stories`)
...
async function fetchLives() {
  try {
    const resp = await window.axios.get(`/users/${authStore.user.id}/lives`)
